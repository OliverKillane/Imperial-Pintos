...
inline fixed32 
int_to_fixed (int32_t n) 
{
    return (fixed32) { .val = n * BINARY_POINT };
}

inline int32_t 
fixed_to_int_floor (fixed32 x) 
{
    return x.val / BINARY_POINT;
}

inline int32_t 
fixed_to_int_round (fixed32 x) 
{
    if (x.val >= 0) {
...
inline fixed32 
add (fixed32 x, fixed32 y) 
{
    return (fixed32) { .val = x.val + y.val };
}

inline fixed32 
add_f_i (fixed32 x, int32_t n) 
{
    return (fixed32) { .val = x.val + (n * BINARY_POINT) };
}

inline fixed32 
sub (fixed32 x, fixed32 y) 
{
    return (fixed32) { .val = x.val - y.val };
}

inline fixed32 
sub_f_i (fixed32 x, int32_t n) 
{
    return (fixed32) { .val = x.val - (n * BINARY_POINT) };
}

inline fixed32 
sub_i_f (int32_t n, fixed32 x) 
{
    return (fixed32) { .val = (n * BINARY_POINT) - x.val };
}

inline fixed32 
mult (fixed32 x, fixed32 y) 
{
    return (fixed32) { .val = (int32_t) (((int64_t) x.val * y.val) / BINARY_POINT) };
}

inline fixed32 
mult_f_i (fixed32 x, int32_t n) 
{
...
inline fixed32 
div_f_i (fixed32 x, int32_t n) 
{
    return (fixed32) { .val = x.val / n };
}

inline fixed32 
div_i_f (int32_t n, fixed32 x) 
{
    return div(int_to_fixed(n), x);
}

inline int32_t 
calc_load_avg (int32_t old_load_avg, int32_t ready_threads) 
{
...
inline int32_t 
calc_priority (int32_t recent_cpu, int32_t nice) 
{
...
inline int32_t 
calc_recent_cpu (int32_t load_avg, int32_t recent_cpu, int32_t nice) 
{
