...
{
	return elem && !elem->prev && elem->next;
}
...
/* Returns the list the elem is part of. */
struct list *get_list(struct list_elem *elem)
{
	while (elem->next)
		elem = elem->next;

	return list_entry(elem, struct list, tail);
}
...
/* Determines if the list the elem resides holds only this element */
bool list_elem_alone(struct list_elem *elem)
{
	return elem->next && elem->prev && !elem->next->next && !elem->prev->prev;
}
