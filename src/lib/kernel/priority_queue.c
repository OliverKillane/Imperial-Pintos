...
bool
pqueue_init (struct pqueue *pq, pqueue_less_func *less, void *aux) 
{
...
/* Pushes a new item onto the priority queue, returns true if successful, 
   otherwise false */
bool
pqueue_push (struct pqueue *pq, void *new) 
{
  if (!pqueue_grow (pq)) 
    return false;
...
/* Retreives the smalles item from the priority queue */
void *
pqueue_top (struct pqueue *pq) 
{
  ASSERT (pq->size);
  return pq->data[1];
}
