...
/* List containing all non-empty ready_queues, in ascending order by priority. */
...
static bool ready_queue_cmp (const struct list_elem *a, const struct list_elem *b, void *aux UNUSED);
...
tqueue_front (void) 
{
  ...
  if (list_empty (&nonempty_ready_queues)) 
    ...
    front_thread = list_entry (
      list_front (
        &list_entry (
          list_front (&nonempty_ready_queues), struct ready_queue, elem)
        ->thread_queue),
      struct thread, elem);
...
tqueue_next (void) 
{
  ...
  {
    struct list *thread_queue = &list_entry (
      list_front (&nonempty_ready_queues), struct ready_queue, elem)->thread_queue;
...
/* if A is lower priority than B, return true. Used for inserting ready_queues 
   ...
   front.*/
static bool 
ready_queue_cmp (const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
...
tqueue_add (struct thread *thread) 
{
  ...
  struct ready_queue* r_list =  &ready_queue_array[thread->priority.priority - PRI_MIN];

  if (list_empty (&r_list->thread_queue))
    list_insert_ordered (&nonempty_ready_queues, &r_list->elem, ready_queue_cmp, NULL);
...
static void
tqueue_priority_update (struct thread *thread, int8_t new_priority)
{
